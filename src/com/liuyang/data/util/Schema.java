...
public final class Schema implements Serializable {
    ...
    // 设置字段值compute计算表达式
    public synchronized void setValueCompute(Function<Row, PrimitveValue> action) {
    	valueComputeAction = action;
    }
    
    // 设置字段值的Reduce运算表达式
    public synchronized void setValueReduce(BiFunction<PrimitveValue, Row, PrimitveValue> action) {
    	
    	valueReduceAction = action;
    }
    
    public synchronized void setValueFilter(Predicate<Row> filter) {
    	valueFilterAction = filter;
    }
    
    public synchronized boolean where(Row row) {
    	return rowFilterAction.test(row);
    }
    
    public synchronized Row compute(Row row) {
    ...
    public synchronized Row reduce(Row a, Row b) {
